...
function requestedCounts() {
  const maximum = seedMaxDocsRaw === undefined || seedMaxDocsRaw === ""
    ? BASE_COUNT
    : parseNonNegativeInteger(seedMaxDocsRaw, "SEED_MAX_DOCS");
  let caps = {};
  ...
  for (const name of COLLECTIONS) {
    ...
    if (Object.prototype.hasOwnProperty.call(caps, name)) {
      cap = Math.min(maximum, parseNonNegativeInteger(caps[name], "cap for " + name));
    }
...
function vectorFor(index) {
  const vector = [];
  for (let d = 0; d < 8; d += 1) {
    vector.push(Number((((index + 1) * (d + 3) % 17) / 17).toFixed(6)));
  }
  return vector;
}
...
async function recreate(db) {
  const existing = new Set(await db.listCollections({}, { nameOnly: true }).toArray().then(rows => rows.map(row => row.name)));
  for (const name of COLLECTIONS) {
...
async function createIndexes(db) {
  ...
  await db.collection("dashboard_metric_snapshots").createIndexes([
    { key: { metric_type: 1, time_granularity: 1, bucket_start: 1, product_line: 1, priority: 1, team_id: 1 }, name: "snapshot_merge_key", unique: true },
